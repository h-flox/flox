...
def test_asyncworkflow_init():
    ...
    wf = AsyncWorkflow(runtime, topology, 1, module, dataset, strategy=DefaultStrategy())
...
def test_asyncworkflow_custom_loss():
    ...
    topology = flat_topology(2)
    runtime = Runtime.simple_setup(max_workers=2)
    wf = AsyncWorkflow(runtime, topology, 1, module, dataset, strategy=DefaultStrategy(), loss_function=custom_loss)
...
def test_asyncworkflow_worker_time_tracker():
    ...
    wf = AsyncWorkflow(runtime, topology, 1, module, dataset, strategy=DefaultStrategy(), worker_time_tracker=tracker)
    wf.start()
