...
(`List[Node]`) and an edge-list (`List[Edge]`) which are then given as inputs
...
def from_adj_list(
    ...
    """Parses an adjacency list into a node list and edge list.
...
def from_adj_matrix(matrix: ArrayLike) -> tuple[list[Node], list[Edge]]:
    """
    ...
    Args:
        matrix (ArrayLike): The adjacency matrix defining the topology.
    ...
    Examples:
        ...
        >>> nodes
        [Node(idx=0, kind=<NodeKind.COORDINATOR: 'coordinator'>, globus_comp_id=None,
    ...
    nodes = [Node(idx=coord, kind=NodeKind.COORDINATOR)]

    for node in others:
        children = list(graph.successors(node))
        kind = NodeKind.WORKER if len(children) == 0 else NodeKind.AGGREGATOR
        nodes.append(Node(idx=node, kind=kind))

    return nodes, edges


def from_dict(data: GraphDict) -> tuple[list[Node], list[Edge]]:
    """
...
def from_edgelist(path: pathlib.Path | str) -> tuple[list[Node], list[Edge]]:
    """
...
def from_networkx(graph: nx.DiGraph) -> tuple[list[Node], list[Edge]]:
    """
...
def from_yaml(path: pathlib.Path | str) -> tuple[list[Node], list[Edge]]:
    """
