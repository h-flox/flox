...
def _node_kind_or_str(value: NodeKind | str) -> NodeKind:
    """
    Validator function for ensuring that a value is of type `NodeKind`.

    Args:
        value (NodeKind | str): Value to validate.

    Returns:
        The validated enum value of `NodeKind`.

    Throws:
        - `ValueError`: If the value is not of type `NodeKind` or `str`, or
            (if the value is a `str`) the value is not a valid `NodeKind` value.
    """
    if isinstance(value, NodeKind):
        ...
        try:
            ...
            legal_values = [e.value for e in NodeKind]  # noqa
            err.add_note(f"Invalid NodeKind value: must be one of {legal_values}.")
            raise err
    else:
        raise ValueError("Invalid value type for NodeKind.")


NodeKindType = t.Annotated[NodeKind, pyd.BeforeValidator(_node_kind_or_str)]
