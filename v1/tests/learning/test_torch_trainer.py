...
from v1.flight import TorchModule, TorchTrainer
from v1.flight.topologies.node import Node
from v1.testing.fixtures import worker_node, worker_state
...
def test_torch_trainer_simple_init(worker_node, worker_state, module_cls, data_cls):
    """
...
def test_torch_trainer_node_init(worker_node, worker_state, module_cls, data_cls):
    """
    Tests a basic setup of using the `TorchTrainer` class for PyTorch-based models.
    """
    model = module_cls()
    data = data_cls()
    trainer = TorchTrainer(
        node=worker_node, strategy=DefaultTrainerStrategy(), max_epochs=1
    )
...
def test_node_device_specifier(worker_node):
    ...
    worker_node["device"] = "cuda"
    trainer = TorchTrainer(
        node=worker_node, strategy=DefaultTrainerStrategy(), max_epochs=1
    )
    assert str(trainer._device) == "cuda"

    worker_node["device"] = "mps"
    trainer = TorchTrainer(
        node=worker_node, strategy=DefaultTrainerStrategy(), max_epochs=1
    )
...
def test_data_module(worker_node):
    ...
    assert isinstance(Foo().train_data(worker_node), DataLoader)
